$('document').ready(function() {
    ...
    $('input[type=checkbox][name=select-all]').change(function() {
        ...
        if ($('input[name=chbox]:checked').length >= 1) {
            $('#table-head a.del').removeClass('disabled');
        } else {
            $('#table-head a.del').addClass('disabled');
        }
    });
    ...
    $('a[name=delete-all]').click(function(evt) {
        ...
        if (confirm('Are you sure you want to delete ' + selected.length + ' items?')) {
            ...
            $.post('items.php', {
                ...
                var item = '';
                for (var a = 0; a < decoded.length; a++) {
                    item = decoded[a];
                    $('input[name=chbox][value=' + item + ']').parent().parent().fadeOut(700, function() {
                        $(this).remove();

                        $('a[name=delete-all], a[name=check-out-all], a[name=check-in-all]').addClass('disabled');
                    });
                }
            });
        }
