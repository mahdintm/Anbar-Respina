...
class Items {
	...
	public function get_items($page, $items_per_page) {
		...
		$res = $this->query("SELECT * FROM anbar ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
	}
	
	public function count_items() {
		$res = $this->query("SELECT COUNT(*) as c FROM anbar", 'count_items()');
	...
	public function count_items_search($string) {
		$s = "%$string%";
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM anbar WHERE id LIKE ? OR name LIKE ? OR descrp LIKE ? OR date_added LIKE ? OR category IN (SELECT id FROM categories WHERE name LIKE ?)", 'count_items_search()');
	...
	public function search($string, $page, $items_per_page) {
		...
		$prepared = $this->prepare("SELECT * FROM anbar WHERE id LIKE ? OR name LIKE ? OR descrp LIKE ? OR date_added LIKE ? OR category IN (SELECT id FROM categories WHERE name LIKE ?) ORDER BY id DESC LIMIT $x,$y", 'search()');
	...
	public function get_category_name($id) {
		$prepared = $this->prepare("SELECT name FROM categories WHERE id=?", 'get_category_name()');
	...
	public function get_item_name($id) {
		$prepared = $this->prepare("SELECT name FROM anbar WHERE id=?", 'get_item_name()');
	...
	public function delete_item($id) {
		$prepared = $this->prepare("DELETE FROM anbar WHERE id=?", 'delete_items()');
		$this->bind_param($prepared->bind_param('i', $id), 'delete_item()');
		$this->execute($prepared, 'delete_item()');
		
		$prepared = $this->prepare("DELETE FROM logs WHERE item=?", 'delete_items()');
		$this->bind_param($prepared->bind_param('i', $id), 'delete_item()');
		$this->execute($prepared, 'delete_item()');
	...
	public function update_item_qty($type, $id, $fromqty, $qty) {
		...
		if($type == 1) {
			$prepared = $this->prepare("UPDATE items SET qty = qty+$qty WHERE id=?", 'update_item_qty()');
		...
		if($update == false) {
			$prepared = $this->prepare("UPDATE items SET qty = $fromqty WHERE id=?", 'update_item_qty()');
			$this->bind_param($prepared->bind_param('i', $id), 'update_item_qty()');
			$this->execute($prepared, 'update_item_qty()');
			return false;
		}
	...
	private function new_log($type, $item, $from, $to) {
		if($type == 1) {
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromqty,toqty,fromprice,date_added,`user`) VALUES(1,?,?,?,((SELECT price FROM anbar WHERE id=?)*?),?,?)", 'new_log()');
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromqty,toqty,fromprice,date_added,`user`) VALUES(2,?,?,?,((SELECT price FROM anbar WHERE id=?)*?),?,?)", 'new_log()');
			$this->bind_param($prepared->bind_param('iiiiisi', $item, $from, $to, $item, $difference, $date, $user), 'new_log()');
			$this->execute($prepared, 'new_log()');
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromprice,toprice,date_added,`user`) VALUES(3,?,(SELECT price FROM anbar WHERE id=?),?,?,?)", 'new_log()');
	...
	public function get_cat_reg_items($catid) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM anbar WHERE category=?", 'get_cat_reg_items()');
	...
	public function get_cat_tot_items($catid) {
		$prepared = $this->prepare("SELECT SUM(qty) as s FROM anbar WHERE category=?", 'get_cat_tot_items()');
	...
	public function new_item($name, $desc, $cat, $qty, $price) {
		...
		$prepared = $this->prepare("INSERT INTO items(name,descrp,category,qty,price,date_added) VALUES(?,?,?,?,?,?)", 'new_item()');
	...
	public function get_item($itemid) {
		$prepared = $this->prepare("SELECT * FROM anbar WHERE id=?", 'get_item()');
	...
	public function update_item($itemid, $name, $desc, $cat, $price) {
		...
		$prepared = $this->prepare("UPDATE items SET name=?, descrp=?, category=?, price=? WHERE id=?", 'update_item()');
