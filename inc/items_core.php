...
class Items {
	...
	public function get_items($page, $items_per_page) {
		...
		$res = $this->query("SELECT * FROM items ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
	}
	
	public function count_items() {
		$res = $this->query("SELECT COUNT(*) as c FROM items", 'count_items()');
	...
	public function count_items_search($string) {
		$s = "%$string%";
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM items WHERE id LIKE ? OR name LIKE ? OR descrp LIKE ? OR date_added LIKE ? OR category IN (SELECT id FROM categories WHERE name LIKE ?)", 'count_items_search()');
	...
	public function search($string, $page, $items_per_page) {
		...
		$prepared = $this->prepare("SELECT * FROM items WHERE id LIKE ? OR name LIKE ? OR descrp LIKE ? OR date_added LIKE ? OR category IN (SELECT id FROM categories WHERE name LIKE ?) ORDER BY id DESC LIMIT $x,$y", 'search()');
	...
	public function get_item_name($id) {
		$prepared = $this->prepare("SELECT name FROM items WHERE id=?", 'get_item_name()');
	...
	public function delete_item($id) {
		$prepared = $this->prepare("DELETE FROM items WHERE id=?", 'delete_items()');
	...
	private function new_log($type, $item, $from, $to) {
		if($type == 1) {
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromqty,toqty,fromprice,date_added,`user`) VALUES(1,?,?,?,((SELECT price FROM items WHERE id=?)*?),?,?)", 'new_log()');
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromqty,toqty,fromprice,date_added,`user`) VALUES(2,?,?,?,((SELECT price FROM items WHERE id=?)*?),?,?)", 'new_log()');
			$this->bind_param($prepared->bind_param('iiiiisi', $item, $from, $to, $item, $difference, $date, $user), 'new_log()');
			$this->execute($prepared, 'new_log()');
			...
			$prepared = $this->prepare("INSERT INTO logs(`type`,item,fromprice,toprice,date_added,`user`) VALUES(3,?,(SELECT price FROM items WHERE id=?),?,?,?)", 'new_log()');
	...
	public function get_cat_reg_items($catid) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM items WHERE category=?", 'get_cat_reg_items()');
	...
	public function get_cat_tot_items($catid) {
		$prepared = $this->prepare("SELECT SUM(qty) as s FROM items WHERE category=?", 'get_cat_tot_items()');
	...
	public function get_item($itemid) {
		$prepared = $this->prepare("SELECT * FROM items WHERE id=?", 'get_item()');
