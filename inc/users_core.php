...
class Users {
	...
	public function get_users($page, $items_per_page) {
		...
		if($role == 1)
			$q = $this->query("SELECT * FROM users ORDER BY id DESC LIMIT $x,$y", 'get_users()');
		elseif($role == 2)
			$q = $this->query("SELECT * FROM users WHERE role=3 OR role=4 ORDER BY id DESC LIMIT $x,$y", 'get_users()');
		elseif($role == 3)
			$q = $this->query("SELECT * FROM users WHERE role=4 ORDER BY id DESC LIMIT $x,$y", 'get_users()');
		else
			return false;
	...
	public function search($string, $page, $items_per_page) {
		...
		$role = $this->session->get_user_role();
		if($role == 1)
			$prepared = $this->prepare("SELECT * FROM users WHERE username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ? ORDER BY id DESC LIMIT ?,?", 'search()');
		elseif($role == 2)
			$prepared = $this->prepare("SELECT * FROM users WHERE (role=3 OR role=4) AND (username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?) ORDER BY id DESC LIMIT ?,?", 'search()');
		elseif($role == 3)
			$prepared = $this->prepare("SELECT * FROM users WHERE (role=4) AND (username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?) ORDER BY id DESC LIMIT ?,?", 'search()');
		else
			return false;
	...
	public function count_users() {
		$role = $this->session->get_user_role();
		if($role == 1)
			$res = $this->query("SELECT COUNT(*) as c FROM users", 'count_users()');
		elseif($role == 2)
			$res = $this->query("SELECT COUNT(*) as c FROM users WHERE role=3 OR role=4", 'count_users()');
		elseif($role == 3)
			$res = $this->query("SELECT COUNT(*) as c FROM users WHERE role=4", 'count_users()');
		else
			return 0;
	...
	public function count_users_search($string) {
		...
		$role = $this->session->get_user_role();
		if($role == 1)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM users WHERE username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?", 'count_users_search()');
		elseif($role == 2)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM users WHERE (role=3 OR role=4) AND (username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?)", 'count_users_search()');
		elseif($role == 3)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM users WHERE role=4 AND (username LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?)", 'count_users_search()');
		else
			return 0;
	...
	public function delete_user($userid) {
		$prepared = $this->prepare("DELETE FROM users WHERE id=?", 'delete_user()');
	...
	public function new_user($name, $username, $password, $email, $role) {
		$prepared = $this->prepare("INSERT INTO users(username,password,name,email,role,date_added) VALUES(?,?,?,?,?,?)", 'new_user()');
	...
	public function user_exists($username) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM users WHERE username=?", 'user_exists()');
	...
	public function userid_exists($userid) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM users WHERE id=?", 'userid_exists()');
	...
	public function get_user($userid) {
		$prepared = $this->prepare("SELECT * FROM users WHERE id=?", 'get_user()');
	...
	public function edit_user($userid, $name, $email, $role) {
		$prepared = $this->prepare("UPDATE users SET name=?, email=?, role=? WHERE id=?", 'edit_user()');
	...
	public function update_user($userid, $name, $email) {
		if($name == false && $email == true) {
			$prepared = $this->prepare("UPDATE users SET email=? WHERE id=?", 'update_user()');
			$this->bind_param($prepared->bind_param('si', $email, $userid), 'update_user()');
		}elseif($name == true && $email == false) {
			$prepared = $this->prepare("UPDATE users SET name=? WHERE id=?", 'update_user()');
			$this->bind_param($prepared->bind_param('si', $name, $userid), 'update_user()');
		}elseif($name == true && $email == true) {
			$prepared = $this->prepare("UPDATE users SET name=?, email=? WHERE id=?", 'update_user()');
	...
	public function update_pass($userid, $pass) {
		$pass = md5($pass);
		$prepared = $this->prepare("UPDATE users SET password=? WHERE id=?", 'update_pass()');
