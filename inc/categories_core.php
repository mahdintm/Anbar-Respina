...
class Categories {
	...
	public function get_cats($page, $items_per_page) {
		...
		$y = $items_per_page;
		$q = $this->query("SELECT * FROM categories ORDER BY id DESC LIMIT $x,$y", 'get_cats()');
		return $q;
	}
	...
	public function search($string, $page, $items_per_page) {
		...
		$prepared = $this->prepared("SELECT * FROM categories WHERE id LIKE ? OR name LIKE ? OR place LIKE ? OR descrp LIKE ? OR date_added LIKE ? ORDER BY id DESC LIMIT $x,$y", 'search()');
	...
	public function count_cats() {
		$res = $this->query("SELECT COUNT(*) as c FROM categories", 'count_cats()');
	...
	public function count_cats_search($string) {
		$string = "%$string%";
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM categories WHERE id LIKE ? OR name LIKE ? OR place LIKE ? OR descrp LIKE ? OR date_added LIKE ?", 'count_cats_search()');
	...
	public function delete_cat($catid) {
		$prepared = $this->prepare("DELETE FROM categories WHERE id=?", 'delete_cat()');
	...
	public function get_cats_dropdown() {
		$q = $this->query("SELECT id,name FROM categories", 'get_cats_dropdown()');
		return $q;
	}
	
	public function new_cat($name, $place, $desc) {
		$date = date('Y-m-d');
		$prepared = $this->prepare("INSERT INTO categories(name,place,descrp,date_added) VALUES(?,?,?,?)", 'new_cat()');
	...
	public function edit_cat($catid, $name, $place, $desc) {
		$prepared = $this->prepare("UPDATE categories SET name=?, place=?, descrp=? WHERE id=?", 'edit_cat()');
	...
	public function get_cat($catid) {
		$res = $this->query("SELECT * FROM categories WHERE id=$catid", 'get_cat()');
		$obj = $res->fetch_object();
		return $obj;
	}
