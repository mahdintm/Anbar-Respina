					...
					<form method="post" action="" name="new-user">
						...
						<div class="select-holder">
							...
							<select name="nuser-role">
								<?php
								if($role == 1){
									echo '<option value="1">مدیرت کل</option>';
									echo '<option value="2">هلپ دسک</option>';
								}
								if($role == 1 || $role == 2)
									echo '<option value="3">رابط</option>';
								if($role == 1 || $role == 2 || $role == 3)
									echo '<option value="4">انبار دار</option>';
								?>
							</select>
						</div>
						<input type="submit" name="nuser-submit" class="ni btn blue" value="ثبت" />
					</form>
				</div>
			</div>
		</div>
